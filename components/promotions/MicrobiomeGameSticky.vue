...
type SavedMicrobiomeGameResult = {
  version: 1
  userId: string | null
  filling: number
  discountPct: number
...
const GAME_RESULT_STORAGE_PREFIX = 'daigo:microbiome-game:result'
const GAME_RESULT_GUEST_STORAGE_KEY = `${GAME_RESULT_STORAGE_PREFIX}:guest`
const GAME_RESULT_STORAGE_TTL_MS = 1000 * 60 * 60 * 24 * 30
...
const panelTitle = computed(() => {
  if (discountWon.value !== null) return couponApplied.value ? 'Промокод применён' : 'Игра завершена'
...
function getGameResultStorageKey(userId: string | number | null | undefined = authStore.userId) {
  const normalizedUserId = String(userId || '').trim()
  return normalizedUserId ? `${GAME_RESULT_STORAGE_PREFIX}:user:${normalizedUserId}` : GAME_RESULT_GUEST_STORAGE_KEY
}

function readSavedGameResult(key = getGameResultStorageKey()): SavedMicrobiomeGameResult | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<SavedMicrobiomeGameResult>
    const expiresAt = Number(parsed.expiresAt || 0)

    if (parsed.version !== 1 || !expiresAt || expiresAt <= Date.now()) {
    ...
    return {
      ...
      filling: Number.isFinite(Number(parsed.filling)) ? Number(parsed.filling) : 0,
      discountPct: Number.isFinite(Number(parsed.discountPct)) ? Number(parsed.discountPct) : 0,
      couponCode: parsed.couponCode ? String(parsed.couponCode) : null,
      couponApplied: Boolean(parsed.couponApplied),
      playedAt: Number(parsed.playedAt || Date.now()),
      expiresAt,
    }
  } catch (error) {
    console.warn('[microbiome-game] saved result restore failed', error)
    return null
  }
}

function saveGameResultToStorage(params: {
  filling: number
  discountPct: number
  couponCode?: string | null
  couponApplied?: boolean
  ...
  const userId = authStore.userId ? String(authStore.userId) : null
  const now = Date.now()
  const result: SavedMicrobiomeGameResult = {
    version: 1,
    userId,
    filling: Number.isFinite(Number(params.filling)) ? Number(params.filling) : 0,
    discountPct: Number.isFinite(Number(params.discountPct)) ? Number(params.discountPct) : 0,
    couponCode: params.couponCode?.trim() || null,
    couponApplied: Boolean(params.couponApplied),
    playedAt: now,
    expiresAt: now + GAME_RESULT_STORAGE_TTL_MS,
  }

  try {
    window.localStorage.setItem(getGameResultStorageKey(userId), JSON.stringify(result))
    if (userId) window.localStorage.removeItem(GAME_RESULT_GUEST_STORAGE_KEY)
  } catch (error) {
    console.warn('[microbiome-game] saved result write failed', error)
  }
}

function hydrateGameResult(result: SavedMicrobiomeGameResult) {
  stopLoop()
  isPlaying.value = false
  isFinished.value = true
  isWon.value = result.discountPct > 0
  discountWon.value = result.discountPct
  promoCode.value = result.couponCode
  couponApplied.value = result.couponApplied
  couponApplyError.value = ''
  promoCopied.value = false
  isRewardSubmitting.value = false
  timeLeft.value = 0
}

function restoreSavedGameResult() {
  const result = readSavedGameResult()
  if (!result) return
  hydrateGameResult(result)
}

let isSubmittingSavedResultAfterAuth = false

async function submitSavedGuestResultAfterAuth() {
  if (!authStore.isAuthenticated || !authStore.userId || isSubmittingSavedResultAfterAuth) return

  const savedUserResult = readSavedGameResult(getGameResultStorageKey(authStore.userId))
  if (savedUserResult?.couponCode) {
    hydrateGameResult(savedUserResult)
    return
  }

  const savedGuestResult = readSavedGameResult(GAME_RESULT_GUEST_STORAGE_KEY)
  if (!savedGuestResult || savedGuestResult.couponCode) return

  isSubmittingSavedResultAfterAuth = true
  hydrateGameResult(savedGuestResult)

  try {
    await sendGameResult(savedGuestResult.filling)
  } finally {
    isSubmittingSavedResultAfterAuth = false
  }
}
...
function finishGame(won: boolean) {
  ...
  const filling = energyPercent.value
  const discountPct = currentDiscount.value
  discountWon.value = discountPct
  saveGameResultToStorage({ filling, discountPct, couponCode: promoCode.value, couponApplied: couponApplied.value })
...
async function sendGameResult(filling: number) {
  if (!authStore.isAuthenticated || !authStore.userId) {
    const discountPct = Number(discountWon.value ?? currentDiscount.value)
  ...
  try {
    const response = await $fetch<{ coupon_code: string; discount_pct: number }>(`/api/shop/promotion/${authStore.userId}/play`, {
      method: 'POST',
      headers: authHeaders(),
      body: {
    ...
    const discountPct = Number(response.discount_pct ?? discountWon.value ?? currentDiscount.value)
    discountWon.value = discountPct

    const code = response.coupon_code?.trim() ?? ''
    promoCode.value = code || null
    saveGameResultToStorage({ filling, discountPct, couponCode: promoCode.value, couponApplied: false })
...
async function applyPromoToCart(code: string) {
  ...
  try {
    ...
    couponApplyError.value = error?.message || 'Не удалось применить промокод к корзине автоматически'
  } finally {
    saveGameResultToStorage({
      filling: energyPercent.value,
      discountPct: Number(discountWon.value ?? currentDiscount.value),
...
async function copyPromoCode() {
  ...
  try {
    ...
    // clipboard API недоступен — пользователь скопирует вручную
  }
}
...
watch(
  ...
  ([isAuthenticated]) => {
    ...
    void submitSavedGuestResultAfterAuth()
  },
  { flush: 'post' },
)

onMounted(() => {
  restoreSavedGameResult()
          ...
          <div class="micro-game__playground">
            <template v-if="!isPlaying">
              <div class="micro-game__intro">
                ...
                <p v-else-if="discountWon !== null" class="micro-game__status" :class="{ 'micro-game__status_success': couponApplied }">
                  {{ couponApplied ? 'Готово!' : 'Игра завершена.' }} Ваша скидка: {{ discountWon }}%.
                </p>
                ...
                <div v-if="promoCode" class="micro-game__promo">
                  <p class="micro-game__promo-label">Ваш промокод</p>
                  <div class="micro-game__code">
                    <span>{{ promoCode }}</span>
                    <button type="button" @click="copyPromoCode">
                      {{ promoCopied ? 'Скопировано!' : 'Скопировать' }}
                    </button>
                  </div>
                  <p v-if="isCouponApplying" class="micro-game__status">
                    Применяем к корзине…
                  </p>
                  <p v-else-if="couponApplied" class="micro-game__status micro-game__status_success">
                    Промокод применён к корзине
                  </p>
                  <p v-else-if="couponApplyError" class="micro-game__error">
                    {{ couponApplyError }}
                  </p>
                </div>
