...
const isSummerArticle = computed(() => slug.value === 'leto-eto-ne-tolko-otdyh')
...
function splitSummerArticleContentTop(html?: string) {
  ...
  const introMatch = source.match(/^\s*(<div\s+class=["']article-summer-intro["'][\s\S]*?<\/div>)\s*/i)
...
const summerTopParts = computed(() => splitSummerArticleContentTop(article.value?.contentTop))
const summerIntroHtml = computed(() => isSummerArticle.value ? summerTopParts.value.intro : '')
const summerWideHtml = computed(() => isSummerArticle.value ? summerTopParts.value.wide : '')
</script>
...
<template>
  <BaseContainer>
    <div class="container mx-auto py-2 lg:py-4">
      ...
      <p v-if="isSummerArticle && article?.preview" class="mt-4 text-base md:text-2xl font-medium leading-snug">
        {{ article.preview }}
      </p>

      <div v-if="isSummerArticle" class="mt-4 flex flex-wrap items-center gap-3 text-sm text-black/70">
        <span class="inline-flex items-center gap-1.5">
          <svg class="h-4 w-4 text-[#4F8EF7]" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M7 3v3M17 3v3M4 9h16M6 5h12a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          {{ formatArticleDate(article?.date) }}
        </span>
        <span class="inline-flex items-center gap-1.5">
          <svg class="h-4 w-4 text-[#4F8EF7]" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 7v5l3 2M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
          {{ article?.time }} мин
        </span>
        <span class="inline-flex items-center gap-1.5">
          <svg class="h-4 w-4 text-[#4F8EF7]" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z" stroke="currentColor" stroke-width="1.8"/><path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" stroke="currentColor" stroke-width="1.8"/></svg>
          {{ article?.views }}
        </span>
        <span class="inline-flex items-center gap-1.5">
          <svg class="h-4 w-4 text-[#4F8EF7]" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M5 5h14v10H8l-3 3V5Z" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg>
          {{ article?.comments }}
        </span>
      </div>

      <div class="mt-4 md:mt-6 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
        <!-- main -->
        <main class="lg:col-span-8 space-y-8">
          ...
          <p v-if="!isSummerArticle" class="xs-max:text-base text-base sm:text-lg font-medium mt-10 sm:mt-16 border-y py-4 w-full">БАД. НЕ ЯВЛЯЕТСЯ ЛЕКАРСТВЕННЫМ СРЕДСТВОМ</p>
          ...
          <YouWillLearn v-if="!isSummerArticle" :key="slug" :container-ids="['article-top','article-bottom']" class="py-0 md:py-6" />
          ...
          <!-- content bottom -->
          <section v-if="article?.contentBottom && !isSummerArticle" class="prose max-w-none mt-12 prose-img:rounded-xl" id="article-bottom">
            <div v-html="article?.contentBottom" />
          </section>

          <p v-if="!isSummerArticle" class="xs-max:text-base text-base sm:text-lg font-medium mt-10 sm:mt-16 border-y py-4 w-full">Данная публикация носит информационный характер. Указанные эффекты основаны на данных исследований, но не являются утверждением о лечении заболеваний.
Биологически активные добавки не являются лекарственными средствами.</p>
          ...
          <!-- FAQ -->
          <section v-if="visibleFaqItems.length && !isSummerArticle" class="mt-16 flex flex-col gap-6">
        ...
        <aside class="lg:col-span-4">
          ...
          <!-- Покупают вместе -->
          <div v-if="!isSummerArticle && ((bundledByIds && bundledByIds.length) || (bundledFallback && bundledFallback.length))" class="mt-12 hidden sm:block">
      ...
      <section v-if="isSummerArticle && visibleFaqItems.length" class="mt-16 flex flex-col gap-6">
        <h2 class="text-xl md:text-product font-medium">Часто задаваемые вопросы</h2>
        <div class="w-full flex flex-col">
          <AccordionItem
            v-for="faqItem in visibleFaqItems"
      ...
      <section v-if="isSummerArticle" class="article-summer-share mt-12 py-5">
        ...
        <h2 class="text-xl md:text-cardhead font-medium !leading-tight">Понравилась статья?</h2>
        <p class="mt-2 text-sm md:text-lg">Поделитесь статьёй с друзьями в социальных сетях</p>
