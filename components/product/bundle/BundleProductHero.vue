...
const itemName = (item: ProductVariantItem) => String(item.name || "").trim();
...
const resolveRequestedVariantId = () => {
  const rawQuery = Array.isArray(route.query.variant)
    ? route.query.variant[0]
    : route.query.variant;
  const requestedVariant = String(rawQuery || "").trim().toLowerCase();

  if (requestedVariant === "x2") {
    return (
      variants.value.find((variant) => omegaQuantity(variant) >= 2)
        ?.variant_id || resolveDefaultVariantId()
    );
  }

  if (requestedVariant === "x1") {
    return (
      variants.value.find((variant) => omegaQuantity(variant) === 1)
        ?.variant_id || resolveDefaultVariantId()
    );
  }

  return (
    variants.value.find((variant) => variant.variant_id === requestedVariant)
      ?.variant_id || resolveDefaultVariantId()
  );
};

watch(
  [variants, () => route.query.variant],
  () => {
    selectedVariantId.value = resolveRequestedVariantId();
  },
  { immediate: true },
);
...
const quantityInCart = computed(() => {
  const variantId = selectedVariant.value?.variant_id;
  const item = cartStore.items.find(
    (i) =>
      String(i.id) === productId.value &&
...
const bundleUi = computed(() =>
  isOmegaBundleSlug(props.product.slug)
    ? OMEGA_BUNDLE_UI[props.product.slug]
    : null,
);

const variantImage = (variant: ProductVariant) => {
  if (!bundleUi.value) return variant.image || coverImage.value;
  return omegaQuantity(variant) >= 2
    ? bundleUi.value.twoImage
    : bundleUi.value.oneImage;
};
  ...
  <section class="mb-8 py-5 sm:mb-10 sm:py-6 xl:py-10">
    <div class="grid items-start gap-6 md:grid-cols-2 xl:gap-10">
      ...
      <div class="flex min-w-0 flex-col">
        <h1
          class="mt-2 text-2xl font-medium !leading-[3.1rem] sm:mt-0 sm:text-3xl xl:text-product"
        >
          {{ name || product.title }}
        </h1>
        <h2
          v-if="product.subtitle"
          class="mt-3 text-base font-medium sm:text-xl"
        >
          {{ product.subtitle }}
        </h2>

        <p
          class="mt-8 whitespace-pre-line text-sm leading-relaxed sm:text-base xl:text-lg font-medium"
        >
        ...
        <p class="w-full py-4 text-sm font-medium xs-max:text-xs">
          ...
          <div class="mt-3 grid gap-3 sm:grid-cols-2 sm:gap-4">
            ...
            >
              ...
              <span class="block text-xl font-medium leading-tight sm:text-2xl">
                {{
                  variant.title ||
                  `${bundleUi?.title || "Набор"} ${omegaQuantity(variant)}`
                }}
              </span>

              <span
                class="mt-3 block w-[calc(100%-118px)] border-t border-black/15"
                aria-hidden="true"
              />

              <span
                class="mt-3 block min-h-[48px] pr-[112px] text-sm leading-snug sm:text-base"
              >
                <span class="block"
                  >{{ omegaQuantity(variant) }} × Омега-3</span
                >
              ...
              <!-- <span -->
              <!--   v-if=" -->
              <!--     variant.benefitLabel || variantDiscountPercent(variant) > 0 -->
              <!--   " -->
              <!--   class="mt-3 inline-flex items-center gap-1 rounded-md bg-[#0DBD27] px-2 py-1 text-xs font-medium text-white sm:text-sm" -->
              <!-- > -->
              <!--   <span aria-hidden="true">🔥</span> -->
              <!--   {{ -->
              <!--     variant.benefitLabel || -->
              <!--     `Выгода ${variantDiscountPercent(variant)}%` -->
              <!--   }} -->
              <!-- </span> -->

              <div class="mt-2 flex flex-col gap-1 absolute bottom-1">
                <!-- <div -->
                <!--   class="mt-2 flex flex-row items-center justify-center text-white gap-1.5 bg-[#16B819] rounded-md px-3 py-0.5" -->
                <!-- > -->
                ...
                <span class="flex flex-wrap items-baseline gap-2">
                  <span
                    v-if="
                      Number(variant.originalPrice || variant.oldPrice || 0) >
                  ...
                  >
                    {{
                      formatMoney(
                        Number(variant.originalPrice || variant.oldPrice),
                      )
                    }}
                    ₽
                  </span>
        ...
        <div class="mt-7 flex flex-wrap items-baseline gap-3">
          ...
          <span
            class="text-3xl font-medium text-cgreen sm:text-4xl xl:text-product"
          >
        ...
        <div id="product-cta" class="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          >
            <template #icon>
              ...
              >
                <path
                  d="M0 5a1 1 0 0 1 1-1h3a1 1 0 0 1 .97.758L5.78 8H29a1 1 0 0 1 .97 1.242l-3 12A1 1 0 0 1 26 22H8a1 1 0 0 1-.97-.758L3.22 6H1a1 1 0 0 1-1-1Zm6.28 5 2.5 10h16.44l2.5-10H6.28ZM10 26a2 2 0 1 1 0 4 2 2 0 0 1 0-4Zm14 0a2 2 0 1 1 0 4 2 2 0 0 1 0-4Z"
                />
              </svg>
            </template>
          ...
          <div
            v-else
            class="flex w-full items-center justify-between gap-2 rounded-lg bg-primary px-2 h-11 md:h-12 text-white"
          >
            <button
              type="button"
              :disabled="adding"
