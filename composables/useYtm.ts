...
export function useYtm() {
  ...
  const viewListing = (listing: ListingObject) =>
    push({ event: 'view_item_list', listing })

  const viewDetail = (detail: DetailObject) =>
    push({ event: 'view_item', detail })

  const viewCart = (cart: CartObject) =>
    push({ event: 'view_cart', cart })
  ...
  // 🔹 promoClick теперь принимает МАССИВ промо-объектов (как в PromotionItem.vue)
