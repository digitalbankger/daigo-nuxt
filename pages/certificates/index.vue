...
useHead({
  title: 'Сертификаты на продукцию — Daigo',
  meta: [
    { name: 'description', content: 'Сертификаты на продукцию Daigo' },
    { property: 'og:title', content: 'Сертификаты на продукцию — Daigo' },
    { property: 'og:description', content: 'Сертификаты на продукцию Daigo' }
  ]
})
</script>
  ...
  <BaseContainer>
    <section class="relative w-full">
      <div class="flex flex-row items-center justify-between mb-6">
        <h1 class="text-slider font-medium">Сертификаты на продукцию</h1>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-6">
        <NuxtLink
          v-for="cat in store.categories"
          :to="`/certificates/${cat.slug}`"
          :key="cat.id"
