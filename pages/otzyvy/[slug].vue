...
/** SSR-загрузка всех отзывов один раз */
const { pending, error } = await useAsyncData(
  'reviews:all',
  async () => {
    if (!reviewsStore.isLoaded) await reviewsStore.loadAllReviews()
    return true
  }
)
...
<template>
  <BaseContainer>
    <section class="relative w-full">
      <h1 class="text-[clamp(2.2rem,5vw,3.2rem)] font-medium mb-8">{{ conf!.h1 }}</h1>

      <div v-if="pending" class="text-center text-gray-500">Загружаем…</div>
      <div v-else-if="error" class="text-center text-red-500">Не удалось загрузить отзывы</div>
      ...
      <template v-else>
        ...
        <div v-if="totalPages > 1" class="flex items-center justify-center gap-2 mt-8">
          <button class="px-3 py-2 rounded border" :disabled="page <= 1" @click="go(page - 1)">← Назад</button>
          <span class="mx-2">Стр. {{ page }} из {{ totalPages }}</span>
          <button class="px-3 py-2 rounded border" :disabled="page >= totalPages" @click="go(page + 1)">Вперёд →</button>
        </div>
      </template>
