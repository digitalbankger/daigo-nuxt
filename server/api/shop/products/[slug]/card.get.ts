...
import { MOCKED_SLUGS } from '../../mockedSlugs'
...
export default defineEventHandler(async (event) => {
  ...
  // ===============================
  // 1) Если slug — моковый → вернуть mock
  // ===============================
  if (MOCKED_SLUGS.includes(slug)) {
    const mock = mockProducts.find(p => p.slug === slug)
    if (!mock) {
      throw createError({ statusCode: 404, statusMessage: 'Mocked product not found' })
    }
    return adaptToProduct(mock)
  }

  // ===============================
  // 2) Иначе — обычный реальный API
  // ===============================
